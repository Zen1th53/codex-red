...
2. **YAML frontmatter is required.** Skills without it won't load via the Codex Skills system.
...
5. **Use code blocks with language tags.** It's how Codex (and humans) parse them best.
...
6. Update [`codex-skills.json`](codex-skills.json) if it exists (run `python tools/build_manifest.py` if available).
...
- AI-generated skills without operator review (use Codex to draft, then verify and edit)
